...
#define FOGW_SPANCAP 8192          /* the same clamp the native pass applies   */
...
static int fogw_window(char* ta, int* col0, int* row0, int* cols, int* rows)
{
    ...
    if (vw < 64 || vh < 64 || vw > 4096 || vh > 4096) return 0;
    ...
    if (evw > FOGW_SPANCAP) evw = FOGW_SPANCAP;
    if (evh > FOGW_SPANCAP) evh = FOGW_SPANCAP;
    if (evw < vw) evw = vw;
    if (evh < vh) evh = vh;
