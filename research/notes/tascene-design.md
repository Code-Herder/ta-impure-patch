...
| `shadowlen=<a>,<b>` | how long a 3DO's shadow is, in world units: **`a + b × the model's height`**, default `10,0.25` — the kbot lab (11.3 tall) gets 13, a Peewee (26.4) 16.6, the laser tower (46.7) 21.7. A parallel light makes length proportional to height, and the 40° that makes TA's low buildings read gave the kbots three times too much (31 px on a Peewee); the engine does not project mobile units at all — a flat 5-px drop of the silhouette. Implemented as a per-caster vertical scale `sv = (a + b·h) / (h·cot el)` on the unit's shadow-space position, which both the depth pass and the unit's own shadow lookup use, so a unit stays consistent with itself; only the ground sees the real world, and hills are unscaled. `off` = physical, and `shadowlen=off` reproduces the plain 40° frame to the pixel. Measured per caster (shadow-on/off pixels in a 140-px window): Peewee 793 → 413, laser tower 1 158 → 567, extractor 795 → 333, lab 2 110 → 1 452 |
