...
| `shadowsun=<az,el>` | the **shadows'** sun, default `225,60`. The engine's own slant projection as a parallel light is `225,70.53` — `shadows-cloak.md` has it binary-verified as `gx = x + y/4, gy = −z − y/4`, direction `(−1, 4, 1)/√18`, every shadow leaning **up-right** by a quarter of the caster's height — and the default keeps that lean but lowers the elevation so a shadow is 0.58 of the height instead of 0.35, because the engine's stubs read as too short once they are soft (a 26-unit Peewee: 15 px instead of 9). The engine's shading light and its shadow light differ in elevation and azimuth; the lab reproduces both rather than one of them. `shadowsun=225,70.53` is the engine's exactly; `215.5,53.1` makes the shadow fall away from the light the unit is shaded by |
