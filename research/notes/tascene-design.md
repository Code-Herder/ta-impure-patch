...
- **`slant`** (since G14j, 2026-09-07) — a structure: the engine's cached slant projection in
  ...
  render as geometry. A pack built before G14j carries no caster mesh and its structures draw
  ...
  the lab does not have, so their rims were not compared ([roadmap](roadmap.html) G14j).
...
| `shadowsun=<az,el>` | the **shadows'** sun, default `225,40`. The engine's own slant projection as a parallel light is `225,70.53` — `shadows-cloak.md` has it binary-verified as `gx = x + y/4, gy = −z − y/4`, direction `(−1, 4, 1)/√18`, every shadow leaning **up-right** by a quarter of the caster's height — and the default keeps that lean but lowers the elevation so a shadow is 1.2× the height instead of 0.35×. TA's buildings are low: the kbot lab is **11.3 units tall over a 95-unit footprint** (`base` piece; the nano arms reach 14.7), so the engine's slant gives it a 3-px rim, 60° gave 6 px and still read as nothing, 40° gives 13 px and reads. Measured against `shadows=0` on the base fixture: 60° → 6 311 shadowed px of which 1 813 darker than 0.7× lit; 40° → 11 785 / 4 684; 40° with `penumbra=0.05` → 9 266 / 4 808 (fewer washed-out pixels, more dark ones). The engine's shading light and its shadow light differ in elevation and azimuth; the lab reproduces both rather than one of them. `shadowsun=225,70.53` is the engine's exactly; `215.5,53.1` makes the shadow fall away from the light the unit is shaded by |
...
| `penumbra=<k>` | PCSS-lite: the receiver's own texel and the 16 Poisson taps over a 24-unit search radius find the average blocker (**since G14i, 2026-09-06** — 8 ring taps 12–28 texels out missed a kbot's head outright; [renderers](renderers.html) §2.12), and the kernel radius is `k` × the receiver–blocker distance, at least half a texel. Default **0.05** — TA's casters are 11–60 units tall, so 0.25 was all penumbra (median shadow 0.876 of lit; at 0.1 the core reached 0.405 and the median 0.737) and 0.1 still washed out a low building's rim. 0 = hard |
| `shadowlen=<a>,<b>` | how long a 3DO's shadow is, in world units: **`a + b × the model's height`**, default `14,0.25` — a Flash (8.5 tall) gets 16, the kbot lab (11.3) 17, a Peewee (26.4) 20.6, the laser tower (46.7) 25.7. The base term is what a *low* unit shows: at `10,0.25` a Flash's 12-unit shadow shaded as many pixels as a Peewee's (260 vs 153–247 in a 60-px window) but its 28-wide body hid nearly all of it. A parallel light makes length proportional to height, and the 40° that makes TA's low buildings read gave the kbots three times too much (31 px on a Peewee); the engine does not project mobile units at all — a flat 5-px drop of the silhouette. Implemented as a per-caster vertical scale `sv = (a + b·h) / (h·cot el)` on the unit's shadow-space position, which both the depth pass and the unit's own shadow lookup use, so a unit stays consistent with itself; only the ground sees the real world, and hills are unscaled. `off` = physical, and `shadowlen=off` reproduces the plain 40° frame to the pixel. Measured per caster (shadow-on/off pixels in a 140-px window): Peewee 793 → 413, laser tower 1 158 → 567, extractor 795 → 333, lab 2 110 → 1 452 |
...
| `terrainshadow=<b>` | the heightfield casts too, **default 1 here and 0 in the game since 2026-09-09** — a deliberate divergence: the ground casting on itself is the defect of [renderers](renderers.html) §2.7b, the game defaults it off because no bias-shaped fix survived costing, and the lab keeps it on because it is the fixture that fix will be measured in. Originally: The art has slope shading but no cast shadows, so this adds and does not double-count the way relief did; on Two Continents' 53° sun it amounts to 786 pixels |
...
| `pbias=<k>` | bias the receiver's OWN texel by `k` times its plane's depth change across one texel — `k · length(dzduv) · uShScale.z`, taken before the tap clamp. The shipped bias uses `nl` instead, which is 1–3 texels whatever the receiver's real gradient is. Default 0 (off). The nearest thing to a fix measured so far: acne 1.50 → 0.91 at `k=4`, but 12 % of the real shadow goes with it |
| `mindist=<w>` | ignore blockers nearer than `w` world units. A bias-invented blocker should sit a fraction of a unit above the receiver where a real caster's is tens of units up, so this ought to separate them — **and it does not**: 7.66 → 7.48 at 8 units, which says the false blocker is far in depth, not near. Kept for the null result. Default 0 (off) |
