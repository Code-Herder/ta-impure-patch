...
| G15e — Classic++ UI: the UI atlas's restored twin, the palette-validity rule, `uirestore` | ○ **unblocked 2026-09-08** (G15-0's verdict in), next up; builds the colour as a **per-surface** channel on every twin with copies carrying both, per [§13.2](gui-renderer.html)'s amendment — not a screen-space layer, which retained surfaces like `panel+0xBC` cannot use | Q2 bar against G15-0's offline output; sheets judged by the owner; fps unchanged |
