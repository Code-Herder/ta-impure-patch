...
| Terrain tiles, features (trees, rocks) | ○ engine 8bpp | — | G13 |
| Fog overlay, UI, minimap | ○ engine 8bpp (fog is *mirrored* per fragment) | — | G13 / stays engine-side |

So the engine's software frame is now **terrain + features + fog + UI only**; everything that
moves is ours. That is the entry condition for G13.
...
| 6 | **G13 — full scene takeover**: terrain tiles → GL (TNT tile atlas), features → GL sprites, ~~projectiles/explosions native~~ (done, G12e), ~~particle sfx native~~ (done, G12f), fog native; UI/minimap stay engine-side; then free zoom/camera | engine software frame = data only |
