...
| Chat, dialogs, side panel, minimap, top bar — and the whole shell | ● **ours since G15b** ([GL UI renderer](gui-renderer.html)); phase 2 in progress (G17a–d done, G17e open) | G15a–e, G17a–d | Phase E mirrors the engine's own UI draws into GL twins of its surfaces and draws the twins over the world composite; the engine's surface keeps drawing and stays the oracle and the fallback (`strict` removes it, for the harness only). Phase 2 scales it: the mirror through a sharp-bilinear ramp, a device-resolution sharp layer above it, and **the cursor ours since G17c** — 1× device pixels at every `k`, from the true pointer — and **the text ours since G17d**, stamped from TA's own glyphs into the twin. The minimap (G17e) is still the engine's, mirrored at 1× |
