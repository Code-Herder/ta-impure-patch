...
| 6 | **G13 — full scene takeover**: terrain tiles → GL (TNT tile atlas), features → GL sprites, ~~projectiles/explosions native~~ (done, G12e), particle sfx (smoke/fire/wakes/nano — the hook-vector objects) native, fog native; UI/minimap stay engine-side; then free zoom/camera | engine software frame = data only |
