...
| VA | What | How |
| --- | --- | --- |
| `0x48CF30` | issue the order — `(rec, orderByte, &worldPoint, 0, 0, 0)`, `0x48CF30` is the one call both buttons make | its two call sites (`0x499087`, `0x4991F5`) plus live: the click that reaches it walks the unit to the point clicked |
...
`0x492593`, `0x496A6F` (19) and `0x497F9C`, `0x4997DE` (20) put nothing else there — all of
which stock deselects when the order byte is 1.
...
- **`main+0x2CBE` has exactly one behavioural reader.** It has **28** `.text` references: 13
  ...
  `0x49953E`, `0x499581`, `0x49964B`, `0x4997D5`), the save at `0x443702` that `0x4436E0`
  restores, and `0x499027` — the one place the value decides anything. That is why the bug had a
  single 27-byte fix and not a rewrite.
- **`0x498F70` and `0x499100` have one call site each** (`0x4995B3`, `0x49939B`), both inside
  `0x499200`. Neither address appears as a literal elsewhere.
