...
[terrain & depth](terrain-depth.html) §7. Read every frame by `tagpu_native.c`/`tagpu_render3do.c` (uploaded verbatim as a 256×1 RGBA texture; the in-game colours match, which is the byte-order proof) and once per map by the restorer, whose cache key covers it. Read-only. |
