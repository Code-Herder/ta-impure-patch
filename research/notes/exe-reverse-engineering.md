...
| Live palette | `main+0x143A7`: 256 entries × 4 bytes, **R, G, B, pad** — the palette the engine cycles for water. Read every frame by `tagpu_native.c`/`tagpu_render3do.c` (uploaded verbatim as a 256×1 RGBA texture; the in-game colours match, which is the byte-order proof) and once per map by the restorer, whose cache key covers it. Read-only. |
