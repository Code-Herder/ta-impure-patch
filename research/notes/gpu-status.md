...
| **The engine's fog grid is one cell per 32 px of the 1× viewport, and `taFog()` clamps outside it** — so at zoom < 1 the ring beyond that grid takes the grid's edge cells rather than the fog that belongs there. MEASURED 2026-09-09 (`tacli peek` through the descriptor at `main+0x1421F`): **118 × 68** cells for the 3712 × 2096 viewport of a 3840×2160 screen, **78 × 45** for the 2432 × 1376 of a 2560×1440 one — one per 32 px plus two, and the same size whatever the zoom, so at 0.25× the grid covers a sixteenth of the world on screen. Not yet looked at in a fogged game | `tagpu_glsl.h` `taFog`, `main+0x1421F` | the grid is the engine's, built by `0x4843C0` into a descriptor allocated somewhere not yet read (`0x483F1C` only frees it), so widening it is engine work — or sample the LOS/MAPPED source maps in the ring instead, which [terrain & depth](terrain-depth.html) §5.2 measured as NOT reproducing the overlay |
| **`cells` in that descriptor is not always `cols × rows`, and the guard that assumed it was cost the fog its whole rule.** MEASURED 2026-09-09: 3840×2160 reports `cells` 8024 = 118 × 68 exactly, 2560×1440 reports **3512 against 78 × 45 = 3510**. `fogMode` is set inside that test, so a rejected grid is a frame with no fog at all — `native: fog=0 foglut=0` at 2560×1440 with LOS on and mapping off, where the engine was fogging normally. FIXED the same day: the test asks that the allocation COVER what we read (`cells >= cols*rows`) and probes the `cols × rows` we sample rather than the engine's count, which is strictly tighter than the probe it replaced. Where the extra two come from is **not traced** | `tagpu_native.c`, the `OFF_FOGGRID` block | reading the allocation site, if it ever matters |
