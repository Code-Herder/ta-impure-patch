...
| `main+0x14283` / `+0x1428B` | `TILE_SET` `{count, pixels}` and the `u16` `TILE_MAP` — the terrain pass reads both per frame. The GLSL restorer reads them once more when its job starts, on the render thread: each tile's edge texels for the tileability test and the whole tile map once, to rank tiles by distance from the viewport (`tagpu_terr.c` `restore_order`); the pixels themselves are sampled from the R8 atlas already on the GPU. The ONNX path (under `tagpu_restoreonnx.on`) still copies the whole set once per map |
| `main+0x143A7` | the live palette, 256 × (R, G, B, pad) — uploaded per frame by the native passes; snapshotted once per job by the restorer into its own 256×1 texture |
